exports.formatComments = (comments, articles) => {
  let commentsClone = comments.map((element) => {
      ...
      let correctData = articles.find((refElement)=>{return refElement['title'] === elementClone['belongs_to']});
  ...
  exports.formatArticles = (articles) => {
    ...
    articlesClone.forEach((element) => {element.created_at = new Date(element.created_at)
    element.votes = 0}
    )
